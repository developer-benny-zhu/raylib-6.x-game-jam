...
splash_screen_update :: proc(splash: ^Splash_Screen, game_state: ^Game_State) {
	...
	raylib.BeginDrawing()
	raylib.ClearBackground(raylib.Color{10, 8, 18, 255})
	...
	shadow_offset := linalg.Vector2f32{3 * scale, 3 * scale}
	...
	case COOKIE_SCREEN_START ..< COOKIE_SCREEN_END:
		...
		shadow_color := raylib.Color{0, 0, 0, color.a}
		font_size := f32(math.round(40 * scale))

		draw_text(
			"Made by COOKIE POLICE",
			screen_center + shadow_offset,
	...
	case ODIN_SCREEN_START ..< ODIN_SCREEN_END:
		...
		shadow_color := raylib.Color{0, 0, 0, color.a}
		font_size := f32(math.round(36 * scale))
		text_pos := linalg.Vector2f32{screen_center.x, screen_center.y - (180 * scale)}
		...
		logo_shadow_pos := screen_center + linalg.Vector2f32{4 * scale, 4 * scale}
		draw_texture(
			game_state.assets.ginger_bill,
			source,
			logo_shadow_pos,
			.Center,
			tint = raylib.Color{0, 0, 0, u8(f32(color.a) * 0.5)},
			scale = texture_scale,
		)
		draw_texture(
			game_state.assets.ginger_bill,
	...
	case WAR_SCREEN_START ..= WAR_SCREEN_END:
		color := raylib.Color{255, 223, 128, 255}
		update_fade(&color, splash.timer, WAR_SCREEN_START, WAR_SCREEN_END, TRANSITION_TIME)

		shadow_color := raylib.Color{15, 5, 30, color.a}
		font_size := f32(math.round(64 * scale))

		bar_width := 480 * scale
		bar_height := 2 * scale
		bar_y_offset := 70 * scale

		raylib.DrawRectangleV(
			{screen_center.x - (bar_width / 2), screen_center.y - bar_y_offset},
			{bar_width, bar_height},
			raylib.Color{212, 175, 55, u8(f32(color.a) * 0.4)},
		)
		raylib.DrawRectangleV(
			{screen_center.x - (bar_width / 2), screen_center.y + bar_y_offset},
			{bar_width, bar_height},
		...
		draw_text(
			"The War of Hex",
			screen_center + shadow_offset * 1.5,
			raylib.GetFontDefault(),
			font_size,
			.Center,
			tint = shadow_color,
